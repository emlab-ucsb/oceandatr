...
#'
#' @param area_polygon `sf` polygon or multipolygon object of the area of interest (e.g., a country's EEZ)
#' @param spatial_grid `terra::rast()` or `sf` planning grid with the desired resolution and coordinate reference system generated by `get_grid()`. For a raster, values in the planning grid cells should be 1, while all other values are NA
#' @param name name of raster or column in sf object that is returned
#' @param antimeridian `logical` does `area_polygon` or `spatial_grid` span the antimeridian? If so, this should be set to `TRUE`, otherwise set to `FALSE`. If set to `NULL` (default) the function will try to check if data spans the antimeridian and set this appropriately.
#'
