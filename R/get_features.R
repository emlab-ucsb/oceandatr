...
#'
#' @return If an `area_polygon` is supplied, a list of feature data is returned (will be mixed raster and sf objects). If a `spatial_grid` is supplied, a raster stack or `sf` of gridded data is returned, depending on `spatial_grid` format.
#' @export
#'
#' @examples
#' # Grab EEZ data first 
...
#' # Grab all the data
#' features_gridded <- get_features(spatial_grid = planning_grid)

get_features <- function(area_polygon = NULL, spatial_grid = NULL, features = c("bathymetry", "seamounts", "knolls", "geomorphology", "corals", "enviro_regions"), seamount_buffer = 30000, bathy_resolution = 1, antipatharia_threshold = 22, octocoral_threshold = 2, enviro_clusters = NULL, max_enviro_clusters = 6, antimeridian = NULL){
  
  if("bathymetry" %in% features) { 
    message("Getting depth zones...")
    bathymetry <- get_bathymetry(area_polygon = area_polygon, spatial_grid = spatial_grid, resolution = bathy_resolution, antimeridian = antimeridian)
  }
  
  if("seamounts" %in% features) { 
    message("Getting seamount data...")
    seamounts <- get_seamounts_buffered(area_polygon = area_polygon, spatial_grid = spatial_grid, buffer = seamount_buffer, antimeridian = antimeridian)
  }
  
  if("knolls" %in% features) { 
    message("Getting knoll data...")
    knolls <- get_knolls(area_polygon = area_polygon, spatial_grid = spatial_grid, antimeridian = antimeridian)
  }
  
  if("geomorphology" %in% features) { 
    message("Getting geomorphology data...")
    suppressMessages({
      geomorphology <- get_geomorphology(area_polygon = area_polygon, spatial_grid = spatial_grid, antimeridian = antimeridian)
    })
  }
  ...
  if("corals" %in% features) { 
    ...
    suppressMessages({
      corals <- get_coral_habitat(area_polygon = area_polygon, spatial_grid = spatial_grid, antipatharia_threshold = antipatharia_threshold, octocoral_threshold = octocoral_threshold, antimeridian = antimeridian)
    })
  ...
  if("enviro_regions" %in% features) { 
    ...
    suppressMessages({
      enviro_regions <- get_enviro_regions(area_polygon = area_polygon, spatial_grid = spatial_grid, show_plots = FALSE, num_clusters = enviro_clusters, max_num_clusters = max_enviro_clusters, antimeridian = antimeridian)
    })
  }
