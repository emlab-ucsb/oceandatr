...
#'
#' @return If an `area_polygon` is supplied, an `sf` seamounts object. If a `spatial_grid` is supplied, a raster or `sf` of gridded seamount data, depending on `spatial_grid` format.
#' @export
#'
#' @examples
#' # Get EEZ data first 
...
#' seamounts_gridded <- get_seamount_peaks(spatial_grid = planning_grid)
get_seamount_peaks <- function(area_polygon = NULL, spatial_grid = NULL, name = "seamounts", antimeridian = NULL){
  ...
  get_data_in_grid(area_polygon = area_polygon, spatial_grid = spatial_grid, dat = seamounts, meth = meth, name = name, antimeridian = antimeridian)
}
