...
#'
#' @return If `classify_bathymetry = FALSE`, a bathymetry raster or `sf` object for the `area_polygon` or `spatial_grid` depending on which was supplied. 
#' If `classify_bathymetry = FALSE`, a depth zones raster or `sf` object for the `area_polygon` or `spatial_grid` depending on which was supplied. If a `spatial_grid` is supplied, a raster stack of zones or `sf` object with one zone in each column is returned, depending on `spatial_grid` format.
#' @export
#'
#' @examples
#' # Get EEZ data first 
...
get_bathymetry <- function(area_polygon = NULL, spatial_grid = NULL, classify_bathymetry = TRUE, above_sea_level_isNA = FALSE, name = "bathymetry", bathymetry_data_filepath = NULL, resolution = 1, keep = FALSE, path = NULL, download_timeout = 300, antimeridian = NULL){
  ...
  if(is.null(bathymetry_data_filepath)){
    bathymetry <- get_etopo_bathymetry(area_polygon_for_cropping, resolution = resolution, keep = keep, path = path, download_timeout = download_timeout) %>% 
      get_data_in_grid(area_polygon, spatial_grid, dat = ., meth, name, antimeridian)
  } else{
    bathymetry <- get_data_in_grid(area_polygon, spatial_grid, dat = bathymetry_data_filepath, meth, name, antimeridian) 
    }
